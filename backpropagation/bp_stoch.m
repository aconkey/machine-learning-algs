...
for i = 1:n_epoch
	...
	for d = 1:n_inst
		...
		Act_out = logsig([1 Act_hid]*W_out);	% bias node concatenated
		Delta_out = Act_out.*(1.-Act_out).*(Targets-Act_out);
		Delta_hid = Act_hid.*(1.-Act_hid).*(W_out*Act_out');
		W_hid = W_hid+(eta.*Delta_hid.*x');
		W_out = W_out+(eta.*Delta_out.*Act_hid);
		
		% compute error:
		for k = 1:n_out
			err = err+0.5.*(Targets(d,k)-Act_out(d,k)).^2;
		end
	end
end
